...
def get_text(link, domain, ordered_text=0):
    ...
    if domain == "car":
        for div in soup.find_all("div", {"itemprop": "text"}):
            for p in div.find_all("p"):
                if p.text not in paragraphs:
                    text.append(p.text)
            paragraphs.update([p.text for p in div.find_all("p")])
...
def load_domain_terms(domain, limit, clean=0):
    ...
    while counter < limit:
        try:
            with open(path + domain + "/" + str(counter) + ".txt", "r", encoding="utf-8") as file:
